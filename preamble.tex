...
% mismo preámbulo compone "First Numbers": el mismo cuaderno, en inglés,
% con sus cadenas de texto en lang/en.tex en vez de lang/es.tex; y
% polish.tex (y polish-bw.tex), con \booklang{polish}, "Poznaję liczby",
% en polaco, con lang/pl.tex.
\makeatletter
\newif\ifenglishbook
\newif\ifpolishbook
...
\else\ifpolishbook
  ...
  % babel-polish también hace de " un carácter activo (ver más abajo, en
  % el español).
...
\newtcolorbox{cajaJunta}[1][]{cajadia, #1,
  ...
  title=\lblJunta}
% Las de la primavera.
\newtcolorbox{cajaSuma}[1][]{cajadia, #1,
  colback=colorDibujaClaro, colframe=colorDibuja, colbacktitle=colorDibuja,
  title=\lblSuma}
\newtcolorbox{cajaResta}[1][]{cajadia, #1,
  colback=colorCompletaClaro, colframe=colorCompleta, colbacktitle=colorCompleta,
  title=\lblResta}
\newtcolorbox{cajaParte}[1][]{cajadia, #1,
  colback=colorRelacionaClaro, colframe=colorRelaciona, colbacktitle=colorRelaciona,
  title=\lblParte}
\newtcolorbox{cajaDiez}[1][]{cajadia, #1,
  colback=colorRespondeClaro, colframe=colorResponde, colbacktitle=colorResponde,
  title=\lblDiez}
\newtcolorbox{cajaProblema}[1][]{cajadia, #1,
  colback=colorLecturaClaro, colframe=colorLectura, colbacktitle=colorLectura,
  title=\lblProblema}
\newtcolorbox{cajaBloques}[1][]{cajadia, #1,
  colback=colorRespondeClaro, colframe=colorResponde, colbacktitle=colorResponde,
  title=\lblBloques}
% Solo en "First Numbers": los nombres de los números, cuando llegan las
% decenas (thirTEEN, THIRty).
\newtcolorbox{cajaNombres}[1][]{cajadia, #1,
  colback=colorRelacionaClaro, colframe=colorRelaciona, colbacktitle=colorRelaciona,
  title=\lblNombres}
% Las del verano.
\newtcolorbox{cajaTabla}[1][]{cajadia, #1,
  colback=colorRespondeClaro, colframe=colorResponde, colbacktitle=colorResponde,
  title=\lblTabla}
\newtcolorbox{cajaDinero}[1][]{cajadia, #1,
  colback=colorRelacionaClaro, colframe=colorRelaciona, colbacktitle=colorRelaciona,
  title=\lblDinero}
\newtcolorbox{cajaHora}[1][]{cajadia, #1,
  colback=colorDibujaClaro, colframe=colorDibuja, colbacktitle=colorDibuja,
  title=\lblHora}
\newtcolorbox{cajaMide}[1][]{cajadia, #1,
  colback=colorCompletaClaro, colframe=colorCompleta, colbacktitle=colorCompleta,
  title=\lblMide}
...
% El hueco en el que se escribe la respuesta: "10 y 3 son [ ]" (#1, el
% ancho: más estrecho para una cifra sola).
...
\newcommand{\marcoDiez}[1]{%
  ...
  \end{tikzpicture}}
% Del 11 al 20, dos marcos, uno encima de otro: el primero, lleno; del
% 21 en adelante, ya no marcos, sino la tabla de las decenas (D) y las
% unidades (U) -- en "First Numbers", tens (T) y ones (O): lang/en.tex.
\newcommand{\marcosDiez}[1]{%
  ...
  \else
    \marcoDiez{#1}%
  \fi\fi}
% La tabla de decenas y unidades: (#1 - 5)/10, redondeado, es la cifra de
% las decenas (\numexpr redondea, no trunca).
\newcommand{\tablaDU}[1]{%
  \begin{tikzpicture}[x=12mm, y=10mm, line width=0.9pt]
    \draw (0,0) rectangle (1,1) (1,0) rectangle (2,1) (0,1) rectangle (1,1.55) (1,1) rectangle (2,1.55);
    \node[font=\bfseries] at (0.5,1.28) {\lblDecenasCorto};
    \node[font=\bfseries] at (1.5,1.28) {\lblUnidadesCorto};
    \node[font=\LARGE\bfseries] at (0.5,0.5) {\the\numexpr(#1-5)/10\relax};
    \node[font=\LARGE\bfseries] at (1.5,0.5) {\the\numexpr#1-10*((#1-5)/10)\relax};
  \end{tikzpicture}}
...
% las cosas (tools/gen_numeros.py las dibuja más pequeñas). El 100, con
% sus tres cifras, sí se encoge un poco: hasta 48 mm.
...
\newcommand{\numeroDeHoy}[4]{%
  ...
  \ifdim\anchoNumeroDeHoy>48mm \setlength{\anchoNumeroDeHoy}{48mm}\fi
  \begin{cajaNumero}
  ...
  \ajustado{\anchoNumeroDeHoy}{\fuenteNumeroDeHoy\color{colorLectura}#1} &
